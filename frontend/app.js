    ...
    try {
      ...
      items.forEach(item => {
        ...
        if (item.action_category === 'REVIEW') {
          ...
          catColor = '#06b6d4'; catBg = 'rgba(6,182,212,0.15)'; catBorder = 'rgba(6,182,212,0.3)';
        } else if (item.action_category === 'ADMIN') {
          catColor = '#f43f5e'; catBg = 'rgba(244,63,94,0.15)'; catBorder = 'rgba(244,63,94,0.3)';
        }
          ...
          if (sources && sources.length > 0) {
            topDocsHtml = `
              <div style="margin-bottom:12px; padding:10px 12px; border-radius:8px; background:rgba(0,141,199,0.12); border:1px solid rgba(56,189,248,0.25);">
                <div style="font-size:0.72rem; color:#38bdf8; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; margin-bottom:6px;">📖 Matched Knowledge Center Articles (${sources.length})</div>
                ...
                ${kbDocs.map(s => `
                  <div onclick="openDocument(${s.id})" style="padding:8px 12px; border-radius:8px; background:rgba(15,23,42,0.7); border:1px solid rgba(56,189,248,0.2); cursor:pointer; transition:all 0.2s ease; display:flex; flex-direction:column; gap:4px;">
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-top:4px; font-size:0.68rem; color:#94a3b8;">
                      ...
                      <span style="color:#64748b; font-size:0.68rem;">Knowledge Article</span>
                    </div>
                  </div>
                `).join('')}
              </div>
          ...
          topSourcesHtml = `
            <div style="margin-bottom:14px; padding:12px 14px; border-radius:10px; background:linear-gradient(135deg, rgba(0,141,199,0.12), rgba(15,23,42,0.7)); border:1px solid rgba(56,189,248,0.3); box-shadow:0 4px 14px rgba(0,0,0,0.3);">
              <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
                <span style="display:inline-flex; align-items:center; justify-content:center; width:18px; height:18px; border-radius:50%; background:rgba(56,189,248,0.2); color:#38bdf8; font-size:0.75rem; font-weight:bold;">ℹ</span>
...
3. Verified zero latency degradation under load.`
  ...
  function renderAdminDocsTable() {
    ...
    tbody.innerHTML = ''
    if (filtered.length === 0) {
      tbody.innerHTML = '<tr><td colspan="7" style="padding:20px; text-align:center; color:#94a3b8;">No documents match your filter criteria.</td></tr>'
      return
    }
    ...
    filtered.slice(0, 100).forEach(doc => {
      ...
      tr.innerHTML = `
        <td style="padding:10px 14px; color:#fff; font-weight:600; max-width:280px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
        ...
        <td style="padding:10px 14px; color:#94a3b8; font-size:0.75rem; white-space:nowrap;">${doc.created_at || '-'}</td>
      ...
      if (ctx3) {
        ...
        window.chartAuthorLeaderboard = new Chart(ctx3, {
          ...
          options: {
            ...
            plugins: { legend: { display: false } },
            scales: {
              x: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#94a3b8', precision: 0, beginAtZero: true } },
              y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#94a3b8' } }
            }
          }
        });
